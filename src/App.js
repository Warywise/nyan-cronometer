...
import nyanAudio from "./Nyan_Cat.mp3";

class App extends Component {
  changeTimer = (owner, { target: { name } }, increase) => {
    if (increase) {
      owner.setState((prevSt) =>
        prevSt[name] < 59 ? { [name]: prevSt[name] + 1 } : { [name]: 0 }
      );
    ...
    owner.setState((prevSt) =>
      prevSt[name] > 0 ? { [name]: prevSt[name] - 1 } : { [name]: 59 }
    );
  ...
  render() {
    return (
      <div className="App">
        ...
        <audio
          id="nyan-audio"
          // src="https://www.albinoblacksheep.com/audio/mp3/Nyanyanyanyanyanyanya.mp3"
          src={nyanAudio}
          preload="auto"
        />
      </div>
